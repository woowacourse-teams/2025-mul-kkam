...
class NotificationServiceTest extends ServiceTest {
    ...
    private List<Notification> createUnReadNotifications(Member member, LocalDate... dates) {
    ...
    private List<Notification> createReadNotifications(Member member, LocalDate... dates) {
        return Arrays.stream(dates)
                .map(date -> NotificationFixtureBuilder
                        .withMember(member)
                        .createdAt(date)
    ...
    class GetNotificationsAfter {
        ...
        void success_returnsNotificationsWithin7DaysSortedByLatest() {
            ...
            // when
            ReadNotificationsResponse result = notificationService.readNotificationsAfter(request,
                    new MemberDetails(savedMember));

            // then
            List<NotificationResponse> results = result.readNotificationResponses();
            ...
            assertSoftly(softly -> {
                ...
                softly.assertThat(result.nextCursor()).isEqualTo(5);
            });
        }
        ...
        void success_returnsAllWhenDataSizeEqualsRequestSize() {
            ...
            // then
            assertThat(result.readNotificationResponses().size()).isEqualTo(defaultSize);
        }
        ...
        void success_returnsLessDataWhenAvailableDataIsLessThanSize() {
            ...
            // then
            assertThat(result.readNotificationResponses().size()).isEqualTo(notifications.size());
        }
        ...
        void success_returnsFromLatestWhenLastIdIsNull() {
            ...
            // then
            List<NotificationResponse> readNotificationResponses = result.readNotificationResponses();
    ...
    class GetNotificationsCount {
        ...
        private final LocalDateTime clientTime = LocalDateTime.of(2025, 8, 7, 0, 0, 0);

        private void setupNotificationsForCount(Member savedMember) {
            ...
            notificationRepository.saveAll(createReadNotifications(savedMember,
                    LocalDate.of(2025, 8, 1),
                    LocalDate.of(2025, 8, 2),
                    LocalDate.of(2025, 8, 3),
                    LocalDate.of(2025, 8, 4),
        ...
        void success_validMember() {
            ...
            // when
            GetUnreadNotificationsCountResponse result = notificationService.getUnReadNotificationsCount(
