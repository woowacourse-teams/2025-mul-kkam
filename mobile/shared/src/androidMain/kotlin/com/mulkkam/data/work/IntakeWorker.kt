...
class IntakeWorker(
    ...
    override suspend fun doWork(): Result =
        runCatching {
            ...
            val emojiBytes = getEmojiBytes(emojiUrl)
    ...
    private suspend fun IntakeWorker.getEmojiBytes(emojiUrl: String): ByteArray {
        val targetPixelSize = 52f.toPixel(applicationContext).toInt().coerceAtLeast(16)
        val rawBitmap =
            loadBitmapFromUrl(applicationContext, emojiUrl, targetPixelSize, targetPixelSize)
        val scaledBitmap = rawBitmap?.scaleDown(targetPixelSize)
        val emojiBytes = scaledBitmap?.toBytes(CompressFormat.PNG) ?: ByteArray(0)
        return emojiBytes
    }

    private fun Float.toPixel(context: Context): Float =
    ...
    private fun Bitmap.scaleDown(targetPixelSize: Int): Bitmap {
        ...
        return scale(targetPixelSize, newHeight, filter = true)
    }

    private fun Bitmap.toBytes(format: CompressFormat): ByteArray =
