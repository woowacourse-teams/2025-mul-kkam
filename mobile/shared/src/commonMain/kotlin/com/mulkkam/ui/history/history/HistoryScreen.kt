...
import com.mulkkam.ui.history.history.component.HistoryShimmerScreen
...
fun HistoryScreen(
    ...
    val networkErrorMessage = stringResource(Res.string.network_check_error)
    val deleteFailurePastMessage = stringResource(Res.string.history_delete_failure_past)
    val deleteSuccessMessage = stringResource(Res.string.history_delete_success)

    LaunchedEffect(deleteUiState) {
        when (deleteUiState) {
            is MulKkamUiState.Failure -> {
                val error = (deleteUiState as MulKkamUiState.Failure).error
                val message =
                    if (error !is MulKkamError.HistoryError.InvalidDateForDelete) {
                        networkErrorMessage
                    } else {
                        deleteFailurePastMessage
                    }
            ...
            is MulKkamUiState.Success -> {
                snackbarHostState.showMulKkamSnackbar(
                    ...
                    iconResource = Res.drawable.ic_history_check,
                )
            }
            else -> Unit
        }
                            ...
                            .clickable {
                                if (waterIntakeState !is WaterIntakeState.Present) {
                                    coroutineScope.launch {
                                        snackbarHostState.showMulKkamSnackbar(
                                            message = deleteFailurePastMessage,
                                            iconResource = Res.drawable.ic_alert_circle,
                                        )
                                    }
                                    return@clickable
